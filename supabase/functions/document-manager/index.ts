...
serve(async (req) => {
  ...
  try {
    ...
    const { action, fileName, fileContent, category, department, searchQuery, userId }: DocumentRequest = await req.json()

    switch (action) {
      case 'upload':
        if (!fileName || !fileContent || !category || !userId) {
          return new Response(
            JSON.stringify({ error: 'fileName, fileContent, category y userId son requeridos' }),
      ...
      case 'list':
        ...
        const { data: documents, error: listError } = await query.order('created_at', { ascending: false })
      ...
      case 'search':
        ...
        const { data: searchResults, error: searchError } = await supabase
          .from('documents')
          .select('*')
          .or(`file_name.ilike.%${searchQuery}%,category.ilike.%${searchQuery}%,department.ilike.%${searchQuery}%`)
