...
interface AttendanceRequest {
  action: 'check-in' | 'check-out' | 'get-status' | 'get-history' | 'get-report'
  userId?: string
  location?: string
  notes?: string
  startDate?: string
...
serve(async (req) => {
  ...
  try {
    ...
    const { 
      ...
      endDate, 
      department 
    }: AttendanceRequest = await req.json()
    ...
    switch (action) {
      ...
      case 'check-out':
        ...
        const { data: todayRecord, error: findError } = await supabase
          .from('attendance')
          .select('*')
          .eq('user_id', userId)
          .eq('date', today)
        ...
        if (findError || !todayRecord) {
          return new Response(
            JSON.stringify({ error: 'No se encontró un registro de entrada para hoy' }),
      ...
      case 'get-report':
        ...
        const { data: reportData, error: reportError } = await reportQuery.order('date', { ascending: false })
        ...
        return new Response(
          JSON.stringify({ 
            success: true, 
            report: Object.values(userStats),
