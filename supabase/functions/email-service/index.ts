 ...
 */ async function logNotification(emailData, success, messageId, errorMessage) {
  try {
...
function generateSimulationMessageId() {
  ...
  if (req.method === "GET") {
    ...
    if (path.includes("/notifications")) {
      try {
        ...
        let query = supabase.from('notifications').select('*').order('sent_at', {
          ascending: false
