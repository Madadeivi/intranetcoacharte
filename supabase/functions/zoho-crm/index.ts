 ...
 */ async function getCachedCollaborators() {
  try {
    const { data, error } = await supabase.from('collaborators').select('*').eq('active', true).order('last_name', {
      ascending: true
    });
    if (error) {
 ...
 */ async function syncCollaboratorsFromZoho() {
  try {
    ...
    for (const contact of contacts){
      ...
      const { data, error } = await supabase.from('collaborators').upsert(collaborator, {
        onConflict: 'zoho_id',
 ...
 */ async function getCollaboratorsWithCache(forceSync = false) {
  try {
 ...
 */ async function createZohoCRMContact(contactData) {
  const token = await getZohoAccessToken();
  const ZOHO_API_URL = Deno.env.get("ZOHO_API_URL");
  if (!ZOHO_API_URL) {
    throw new Error("ZOHO_API_URL no está configurado");
