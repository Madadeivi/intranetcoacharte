...
serve(async (req) => {
  ...
  try {
    ...
    if (!collaboratorId) {
      return new Response(
        JSON.stringify({ error: 'ID de colaborador requerido' }),
        { 
          status: 400, 
    ...
    let { data: collaborator, error } = await supabaseClient
      .from('collaborator_profile_view')
      .select('*')
    ...
    if (error && error.code === 'PGRST116') {
      const { data: directData, error: directError } = await supabaseClient
        .from('collaborators')
        .select('id, full_name, first_name, last_name, email, title, work_area, hire_date, avatar_url, initials, mobile_phone, internal_registry, status')
        .eq('id', collaboratorId)
        .single()
