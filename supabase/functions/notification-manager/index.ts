...
serve(async (req)=>{
  ...
  try {
    ...
    const { action, title, message, type, recipientId, recipientDepartment, senderId, notificationId, priority } = await req.json();
    switch(action){
      case "send":
        ...
        const { data: notificationData, error: sendError } = await supabase.from("notifications").insert([
          {
            title,
            message,
            type: type || "info",
      ...
      case "broadcast":
        ...
        const { data: broadcastData, error: broadcastError } = await supabase.from("notifications").insert(notifications).select();
        if (broadcastError) {
          return new Response(JSON.stringify({
      ...
      case "list":
        if (!recipientId) {
          return new Response(JSON.stringify({
            error: "recipientId es requerido"
          }), {
            status: 400,
            headers: {
              "Content-Type": "application/json"
        ...
        const { data: userNotifications, error: listError } = await supabase.from("notifications").select(`
            *,
            sender:profiles!notifications_sender_id_fkey(full_name, email)
          `).eq("recipient_id", recipientId).order("created_at", {
          ascending: false
        }).limit(50);
        if (listError) {
          return new Response(JSON.stringify({
      ...
      case "mark-read":
        ...
        const { data: updatedNotification, error: updateError } = await supabase.from("notifications").update({
          read: true,
